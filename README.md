...
**Error Handling** is how we teach a program to react when things go wrong. Instead of crashing unpredictably when a file is missing or a user enters letters instead of numbers, a well-handled program will catch the issue, report it safely, and either recover or shut down gracefully.

## How Rust is Different (No `try-catch`)
In most languages (like Java, Python, or JS), errors are treated as invisible "Exceptions" that explode at runtime using `try-catch` blocks. If you forget to "catch" them, your program crashes.

Rust does **not** have exceptions. Instead, Rust treats errors as **Data**. A function that can fail will honestly return a special Enum called `Result`. The compiler *forces* you to handle the error before the code can even compile.

## Types of Errors in Rust
Rust categorizes errors into two distinct types:

### 1. Unrecoverable Errors (The "Panic")
*   **What it is:** A fatal situation where the program has no logical reason to continue (e.g., the core database is missing, or a memory buffer overflow is detected).
*   **How it works:** It uses the `panic!()` macro. It immediately stops execution, cleans up memory safely, and kills the program to prevent data corruption or security breaches.

### 2. Recoverable Errors (`Result<T, E>`)
*   **What it is:** Expected, everyday problems (e.g., a failed network request, parsing text to a number, or file not found).
*   **How it works:** Functions return the `Result` enum:
    *   `Ok(T)`: Success! Here is your data of type `T`.
    *   `Err(E)`: Failure! Here is the error detail of type `E`.
*   You use `match`, `.unwrap_or()`, or the `?` operator to unpack and handle the result without crashing the program.

---

### Comprehensive Code Examples

### Part 1: Unrecoverable Errors (`panic!`)
...
fn hitung_total_akses(semester_1: &str, semester_2: &str) -> Result<u32, ParseIntError> {
    ...
    // Kamu merangkainya dalam satu baris dengan sangat elegan!
